...
- **Type:** `() => UnlighthouseContext`{lang="ts"}
  ...
  Unlighthouse makes use of a composition API to retain the core state. This allows you to access unlighthouse _anywhere_,
...
- **Type:** `() => Promise<void>`{lang="ts"}
  ...
  Creates a h3 app which uses listhen as a web server.
